...
def log_from_dict(metric_dict, data_type, n_epoch):
    log_dict = {'epoch':n_epoch}
    for metric, values in metric_dict.items():
        log_dict[data_type+'/'+ metric] = values
        print(data_type+'/'+ metric +  ' : {:.3f}'.format(values))
    return log_dict
...
def load_model(checkpoint_dir, model):
    ...
    checkpoint = torch.load(os.path.join(checkpoint_dir, 'checkpoints', 'checkpoint_best.pt'))
    model.load_state_dict(checkpoint['model_state_dict'])
    # optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    epochs = checkpoint['epochs']
    return epochs+1, model

def load_optimizer(checkpoint_dir, optimizer):
